...
//bypass
#define CACHE_BYPASS(cp, key)						\
  (((key >> 24) ^ (key >> 16) ^ (key >> 8) ^ key) & ((cp)->bpsize-1))
...
static void
unlink_btab_ent(struct cache_t *cp,		/* cache to update */
		struct cache_set_t *set,	/* set containing bkt chain */
		struct cache_blk_t *blk)	/* block to unlink */
{
  struct cache_blk_t *prev, *ent;
  int index = CACHE_BYPASS(cp, blk->tag);

  /* locate the block in the hash table bucket chain */
  for (prev=NULL,ent=set->bypass[index];
       ent;
       prev=ent,ent=ent->bypass_next)
    {
      if (ent == blk)
	break;
    }
  ...
  if (!prev)
    {
      /* head of hash bucket list */
      set->bypass[index] = ent->bypass_next;
    }
  else
    {
      /* middle or end of hash bucket list */
      prev->bypass_next = ent->bypass_next;
    }
  ent->bypass_next = NULL;
}

/* insert BLK onto the head of the hash table bucket chain in SET */
static void
link_btab_ent(struct cache_t *cp,		/* cache to update */
	      struct cache_set_t *set,		/* set containing bkt chain */
	      struct cache_blk_t *blk)		/* block to insert */
{
  int index = CACHE_BYPASS(cp, blk->tag);
  ...
  /* insert block onto the head of the bucket chain */
  blk->bypass_next = set->bypass[index];
  set->bypass[index] = blk;
}
...
enum list_loc_t { Head, Tail, middle};

/* insert BLK into the order way chain in SET at location WHERE */
static void
update_way_list(struct cache_set_t *set,	/* set contained way chain */
		struct cache_blk_t *blk,	/* block to insert */
		enum list_loc_t where, struct cache_t *cp)		/* insert location */
{
  ...
  /* else, more than one element in the list */
    ...
    {
      struct cache_blk_t *temp_next,*temp_prev;
      /* middle of list (and not front or end of list) */
      assert(set->way_head != blk && set->way_tail != blk);
      temp_next=blk->way_next;
      temp_prev=blk->way_prev;
      temp_prev->way_next = blk->way_next;
      temp_next->way_prev = blk->way_prev;
    }
  ...
  else //changed added this part to put the new line on the head of probationary
   {
    /*link at the temp position*/
   struct cache_blk_t *temp_prev,*temp_next;
   temp_prev=set->way_tail->way_prev->way_prev->way_prev->way_prev;
   temp_next=set->way_tail->way_prev->way_prev->way_prev;
   temp_prev->way_next=blk;
   temp_next->way_prev=blk;
   blk->way_prev= temp_prev;
   blk->way_next=temp_next;
      /*blk->way_prev = set->way_tail;
      blk->way_next = NULL;
      set->way_tail->way_next = blk;
      set->way_tail = blk;*/
	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  cp->hsize = CACHE_HIGHLY_ASSOC(cp) ? (assoc >> 32) : 0;
  cp->bpsize = 16;
  cp->blk_mask = bsize-1;
  ...
  for (bindex=0,i=0; i<nsets; i++)
	...
	{
	  cp->sets[i].bypass =
	    (struct cache_blk_t **)calloc(cp->bpsize,
					  sizeof(struct cache_blk_t *));
	  if (!cp->sets[i].bypass)
	    fatal("out of virtual memory");
	}
	 ...
	 during random replacement selection) */
      cp->sets[i].blks = CACHE_BINDEX(cp, cp->data, bindex); /*changed*/
...
{
  switch (c) {
  case 's': return SLRU; //changed added slru
  case 'l': return LRU;
	     ...
	     FILE *stream)		/* output stream */
{
  ...
  fprintf(stream,
	  ...
	  : cp->policy == FIFO ? "FIFO"
          : cp->policy == SLRU ? "SLRU" //changed
          : cp->policy == SLRUB ? "SLRUB" //changed
	  : (abort(), ""));
}
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  /*if (CACHE_TAGSET(cp, addr) == cp->last_tagset)  // changed
    {
  ...
  switch (cp->policy) {
  /*changed added slru case*/
  case SLRUB: ;
      int bpindex = CACHE_BYPASS(cp, tag);
      int i=0;
      for (blk=cp->sets[set].bypass[bpindex];
	   blk;
	   blk=blk->bypass_next)
	{
	  if (blk->tag == tag && (blk->status & CACHE_BLK_VALID))
           {
	    unlink_btab_ent(cp, &cp->sets[set], repl);
            i=1;
           }
	}
  ...
  if(repl->b ==0)
  {
   link_btab_ent(cp, &cp->sets[set], repl);
  update_way_list(&cp->sets[set], repl, middle, NULL);
  }
  }
  ...
  case FIFO:
    repl = cp->sets[set].way_tail;
    update_way_list(&cp->sets[set], repl, Head, NULL);
    break;
  case Random:
    {
  ...
  if (blk->way_prev && cp->policy == LRU)
    {
      ...
      update_way_list(&cp->sets[set], blk, Head, NULL);
    }
  if (blk->way_prev && cp->policy == SLRU) //changed added slru case for hit
    {
      /* move this block to head of the way (MRU) list */
      update_way_list(&cp->sets[set], blk, Head, NULL);
    }
  if (blk->way_prev && cp->policy == SLRUB) //changed added slrub case for hit
    {
     blk->b=1;
      /* move this block to head of the way (MRU) list */
      update_way_list(&cp->sets[set], blk, Head, NULL);
...
// cache_fast_hit: /* changed fast hit handler */
		 ...
		 tick_t now)		/* time of cache flush */
{
  ...
  if (blk)
      ...
      if (blk->status & CACHE_BLK_DIRTY)
	{
      ...
      update_way_list(&cp->sets[set], blk, Tail,NULL);
    }
...
/*changed never used
void slru_policy(struct cache_set_t *aset,struct cache_blk_t *blk)
{
struct cache_blk_t *ptr,*temp;
int i;
ptr=aset->way_head;
for(i=0;i<13;i++)
 {
...
void move(struct cache_set_t *aset,struct cache_blk_t *blk)
{
 struct cache_blk_t *temp_prev, *temp_next;
 temp_prev=blk->way_prev;
 temp_next=blk->way_next;
 temp_prev->way_next=temp_next;
 temp_next->way_prev=temp_prev;
 blk->way_next = aset->way_head;
 blk->way_prev = NULL;
 aset->way_head->way_prev = blk;
 aset->way_head = blk;
}*/
