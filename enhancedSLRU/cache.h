...
#define CACHE_HIGHLY_ASSOC(cp)	((cp)->assoc > 24)
...
enum cache_policy {
  ...
  FIFO,
  SLRU,		/* replace the oldest block in the set */
  SLRUB
};
...
struct cache_blk_t
{
 ...
 struct cache_blk_t *bypass_next;//changed
 int b; //changed
...
struct cache_set_t
{
  ...
  struct cache_blk_t **bypass;   //changed
